...
- **State machine**: generic `StateMachine<T>`/`State<T>` (`Assets/Script/Core/StateMachine.cs`), driven by enum states (`Ennemy` uses `EnemyState`).
...
- `Ennemy` is intentionally spelled with double-n (`Assets/Script/Enemies/Ennemy.cs`).
